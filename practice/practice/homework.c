...
{
	char a = -1;
	/*当存a时，先要看数字 -1是正是负，我们可以知道-1是一个负数,存入后得到的数据是：1111 1111；
	以整形输出，需要进行整形提升，整形提升先看变量自身的类型，自身是有符号字符型，所以看符号位，
	符号位是1，所以知道是负数，在8个1前面再补上24个1，所以存入寄存器时得到的结果是：
	11111111111111111111111111111111
	取出数字a，需要先看变量a自身的类型，变量a本身是有符号char类型，看符号位,符号位是1，
	知道这个数是负数，则需要将补码转化成原码，先-1再除了符号位其他位依次取反，
	得到的数为：10000000000000000000000000000001*/

	unsigned char b = -1;
	/*存b时，先看数字-1是正是负，我们可以知道-1是一个负数,存入后得到的数据是：
	1111 1111；以整形输出，需要进行整形提升，整形提升先看变量自身的类型，
	自身是无符号字符型，所以在8个1前面补上24个0，所以存入寄存器时得到的结果是：
	00000000000000000000000011111111
	取出数字a，需要先看变量a自身的类型，变量a本身是无符号char类型，
	所以直接输出结果：00000000000000000000000011111111*/
	printf(" %d , %d\n", a, b);//-1,255
	//如果是将数字输出的话，看以什么形式输出%d----->有符号整形，输出-1；     %u------>无符号整形，无需原反补回位，直接输出，结果是32个1
	printf("%u,%u", a,b);
	system("pause");
	return 0;
